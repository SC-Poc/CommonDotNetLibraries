...
namespace Common
{
    ...
    public static class ReflectionUtils
    {
        ...
        {
            return type.GetTypeInfo().GetCustomAttributes(false).OfType<TAttr>().FirstOrDefault();
        }
        ...
        {
            var attr = instance.GetType().GetTypeInfo().GetCustomAttributes(true).OfType<TAttr>().FirstOrDefault();
            ...
            {
                attr = @interface.GetTypeInfo().GetCustomAttributes(true).OfType<TAttr>().FirstOrDefault();
        ...
        {
            var type = typeof(TMyInterface);
            return assembly.GetTypes().Where(p => type.IsAssignableFrom(p)).Cast<TMyInterface>();
        }
